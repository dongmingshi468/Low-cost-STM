...
class PlotFrame(ttk.Frame):
    def __init__(self, parent, with_toobar=False,  dpi=100.0, width=300, height=300, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.figure = Figure(figsize=(width / dpi,
                                      height / dpi), dpi=dpi)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Image frame settings - 减小图形尺寸
        self.real_time_current_plot_frame = PlotFrame(
            self.image_frame, dpi=80.0, with_toobar=True, width=self.baseline_size, height=self.baseline_size)
        ...
        self.iv_curve_frame.grid(row=1, column=0, padx=5, pady=5)
        ...
        self.scan_dacz_frame.grid(row=0, column=2, padx=5, pady=5)

        self.scan_adc_frame = PlotFrame(
            self.image_frame, dpi=80.0, with_toobar=True, width=self.baseline_size, height=self.baseline_size)
        init_image = np.random.rand(10, 10)
        self.scan_adc_frame.add_image(init_image)
        self.scan_adc_frame.grid(row=1, column=2, padx=5, pady=5)
        ...
        self.control_frames = ttk.Frame(
            self, width=350, height=self.baseline_size * 2)
        ...
        class _DAC_Control(tk.Frame):
            def __init__(self, parent, text, default_value, cmd_func, convert_func, *args, **kwargs):
                ...
                self.input_string_var.set(default_value)
                self.input_entry = tk.Entry(
                    self, textvariable=self.input_string_var, width=10)
                self.input_entry.grid(
                    row=0, column=1, columnspan=1, padx=2, pady=2)

                self.display_var = tk.StringVar()
                self._update_display(default_value)

                self.display = tk.Label(self, textvariable=self.display_var, width=12)
                self.display.grid(row=0, column=2, padx=2, pady=2)
                self.button = ttk.Button(
                    master=self, text=text, command=self.set_value, width=10)
        ...
        class _ButtonWithEntry(tk.Frame):
            def __init__(self, parent, text, default_value_list, cmd_func, display_list=None, entry_width=8, *args, **kwargs):
                super().__init__(parent, *args, **kwargs)
                ...
                for i in range(len(default_value_list)):
                    if display_list and display_list[i]:
                        label = ttk.Label(master=self, text=display_list[i], width=6)
                        label.grid(row=0, column=i+1, sticky=tk.W, padx=2, pady=2)
                    input_string_var = tk.StringVar()
                    input_string_var.set(default_value_list[i])
                    input_entry = tk.Entry(
                        self, textvariable=input_string_var, width=entry_width)
                    input_entry.grid(
                        row=row_button, column=i+1, columnspan=1, padx=2, pady=2)
                    self.input_string_var_list.append(input_string_var)
                    self.input_entry_list.append(input_entry)

                button = ttk.Button(
                    master=self, text=text, command=self._set_values, width=12)
                button.grid(row=row_button, column=0, padx=2, pady=2)
        ...
        class _MultipleButtons(tk.Frame):
            def __init__(self, parent, text_list, func_list, *args, **kwargs):
                ...
                for i in range(len(text_list)):
                    button = ttk.Button(
                        master=self, text=text_list[i], command=func_list[i], width=8)
        ...
        class _ScanControl(tk.Frame):
            def __init__(self, parent, cmd_func, *args, **kwargs):
                ...
                def _create_entry(default_value, row, col):
                    input_string_var = tk.StringVar()
                    input_string_var.set(default_value)
                    input_entry = tk.Entry(
                        self, textvariable=input_string_var, width=8)
                    input_entry.grid(
                ...
                # Control Button
                button = ttk.Button(
                    master=self, text="Scan", command=_set_values, width=10)
                button.grid(row=2, column=0, sticky=tk.W, padx=2, pady=2)

        open_frame = _ButtonWithEntry(button_frame,  "Open", [
            "COM7"],  self.stm.open, entry_width=8)
        open_frame.grid(row=row_number, column=0, pady=2, sticky=tk.W)
        row_number += 1
        ...
        set_all_dac_button = ttk.Button(
            master=button_frame, text="SetAllDAC", command=_set_all_dac, width=15)
        set_all_dac_button.grid(row=row_number, column=0, pady=2, sticky=tk.W)
        row_number += 1

        # Approach
        # Set Approach
        approach_frame = _ButtonWithEntry(button_frame,  "Approach", [
                                          "500", "1"],  self.stm.approach, entry_width=8)
        approach_frame.grid(row=row_number, column=0, pady=2,
                            sticky=tk.W)
        row_number += 1

        # Create a IV Curve Scan control
        iv_curve_frame = _ButtonWithEntry(button_frame,  "PlotIV", [
                                          "31768", "33768", "10"], self._plot_iv_curve, entry_width=8)
        iv_curve_frame.grid(row=row_number, column=0, pady=2,
                            sticky=tk.W)
        ...
        save_curve_frame = _ButtonWithEntry(button_frame,  "Save", [
            "./data/iv_curve_"], _save_iv_curve, entry_width=15)
        save_curve_frame.grid(row=row_number, column=0, pady=2,
                              sticky=tk.W)
        row_number = row_number + 1
        ...
        pid_frame = _ButtonWithEntry(button_frame,  "SetPID", [
            "0.0001", "0.0001", "0.0"], self.stm.set_pid, display_list=["Kp", "Ki", "Kd"], entry_width=8)
        pid_frame.grid(row=row_number, column=0, pady=2,
                       sticky=tk.W)
        row_number = row_number + 1
        # Control const current mode
        start_cons_current_frame = _ButtonWithEntry(button_frame,  "ConstCurrentOn", [
            "1000"], self.stm.turn_on_const_current, entry_width=8)
        start_cons_current_frame.grid(row=row_number, column=0, pady=2,
                                      sticky=tk.W)
        row_number = row_number + 1
        stop_cons_current_frame = _ButtonWithEntry(
            button_frame,  "ConstCurrentOFF", [], self.stm.turn_off_const_current, entry_width=8)
        stop_cons_current_frame.grid(row=row_number, column=0, pady=2,
                                     sticky=tk.W)
        row_number = row_number + 1
        ...
        scan_button_frame.grid(row=row_number, column=0, pady=2,
                               sticky=tk.W)
        row_number = row_number + 1
        ...
        save_image_frame = _ButtonWithEntry(button_frame,  "Save", [
            "./data/image"], _save_scan_image, entry_width=15)
        save_image_frame.grid(row=row_number, column=0, pady=2,
                              sticky=tk.W)
        row_number = row_number + 1

        # Add command and send
        # Create text widget and specify size.
        cmd_text_box = tk.Text(button_frame, height=1, width=20)
        cmd_text_box.grid(row=row_number, column=0, pady=2)
        ...
        send_button = ttk.Button(
            master=button_frame, text="Send", command=_send_cmd, width=15)
        send_button.grid(row=row_number, column=0, pady=2)
        ...
        self.status_label = ttk.Label(self.status_frame,
                                      text="No Updates", relief=tk.RAISED, width=40, wraplength=300)
